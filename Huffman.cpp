...
static void add_bit(char bit, unsigned char* fout) 
{
    if ((bloc & 7) == 0) 
        fout[(bloc >> 3)] = 0;
...
static void Huff_send(node_t* node, node_t* child, unsigned char* fout) 
{
    if (node->parent)
...
int MSG_ReadBitsCompress(const unsigned char* input, int readsize, unsigned char* outputBuf, int outputBufSize) 
{
    ...
    {
        Huff_offsetReceive(msgHuff.tree, &get, (unsigned char*)input, &offset);
