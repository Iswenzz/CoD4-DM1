...
namespace Iswenzz::CoD4::DM1
{
	...
	NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(sprintState_t, sprintButtonUpRequired, sprintDelay, 
		lastSprintStart, lastSprintEnd, sprintStartMaxLength);
	...
	NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(hudelem_t, type, x, y, z, targetEntNum, fontScale, font, alignOrg, 
		alignScreen, color, fromColor, fadeStartTime, fadeTime, label, width, height, materialIndex, 
		offscreenMaterialIdx, fromWidth, fromHeight, scaleStartTime, scaleTime, fromX, fromY, fromAlignOrg, 
		fromAlignScreen, moveStartTime, moveTime, time, duration, value, text, sort, glowColor, fxBirthTime, 
		fxLetterTime, fxDecayStartTime, fxDecayDuration, soundID, flags);
	...
	NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(clientSnapshot_t, valid, snapFlags, serverTime, messageNum, deltaNum, 
		ping, areamask, cmdNum, ps, numEntities, parseEntitiesNum, numClients, parseClientsNum, serverCommandNum);
	...
	NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(LerpEntityStatePrimaryLight, colorAndExp, intensity, radius, 
	...
	NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(LerpEntityStateVehicle, bodyPitch, bodyRoll, steerYaw, materialTime, 
		gunPitch, gunYaw, team);
	NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(LerpEntityStateEarthquake, scale, radius, duration);
	NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(LerpEntityStateTypeUnion, turret, loopFx, primaryLight, player, vehicle, 
		missile, soundBlend, bulletHit, earthquake, customExplode, explosion, explosionJolt, physicsJitter, anonymous);
	...
	NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(entityState_t, number, eType, lerp, time2, otherEntityNum, 
		attackerEntityNum, groundEntityNum, loopSound, surfType, index, ClientNum, iHeadIcon, iHeadIconTeam, 
		solid, eventParm, eventSequence, events, eventParms, weapon, weaponModel, legsAnim, torsoAnim, 
		un1.helicopterStage, un2, fTorsoPitch, fWaistPitch, partBits);
	NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(clientState_t, clientIndex, team, modelindex, attachModelIndex, 
		attachTagIndex, netname, maxSprintTimeMultiplier, rank, prestige, perks, attachedVehEntNum, 
	...
	NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(archivedFrame_t, index, origin, velocity, movementDir, bobCycle, 
		commandTime, angles);
}
